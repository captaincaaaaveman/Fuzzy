...
void fuzzy_time_to_words(int hours, int minutes, char* words, size_t length) {
  ...
  printf("The fuzzy_minutes are %d", fuzzy_minutes);
  
  if ( minutes < fuzzy_minutes )
  {
     remaining -= append_string(words, remaining, "nearly ");
  }
  else if ( (minutes - fuzzy_minutes) < 3 && (minutes - fuzzy_minutes) > 0) 
  {
     remaining -= append_string(words, remaining, "just gone ");
  }
